  ...
  User.findOne({ where: { email: req.body.email } })
    .then((userExists) => {
      ...
      return User.create({
        ...
        return res
          .status(200)
          .json(Response.create(true, "User created.", null));
      });
    })
    .catch((err) => {
      return res
...
exports.signin = (req, res) => {
  ...
  passport.authenticate("local", (err, user, info) => {
    ...
    if (!user) {
      ...
      return res
        .status(200)
        .json(Response.create(true, "Connected.", { token: token }));
    }
  })(req, res);
};
...
exports.signout = (req, res) => {
  ...
  return res.status(200).json(Response.create(true, "Disconnected.", null));
};
...
exports.get = (req, res) => {
  User.findByPk(req.userId)
    .then((user) => {
      ...
      return res
        .status(200)
        .json(Response.create(true, "User data found.", { user: userData }));
    })
    .catch((err) => {
      return res
        .status(500)
  ...
  passport.authenticate("local", (err, user, info) => {
    ...
    user
      ...
      .then(() => {
        return res
          .status(200)
          .json(Response.create(true, "Password was updated.", null));
      })
      .catch((err) => {
        return res
          .status(500)
...
exports.updateNickname = (req, res) => {
  User.findOne({ email: req.body.email })
    .then((user) => {
      ...
      return User.findOne({ nickname: req.body.nickname }).then(
        (nicknameUser) => {
          if (nicknameUser) {
            return res
              .status(200)
          ...
          return user.save().then(() => {
            return res
              .status(200)
              .json(Response.create(true, "Nickname was updated.", null));
          });
        }
      );
    })
